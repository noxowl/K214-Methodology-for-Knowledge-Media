...
class BlockManager {
  ...
  BlockManager(MessageReceiver ownReceiver, Map<Actor, MessageSender> senderMap, PVector gameFrameSize) {
    ...
    IntStream.range(0, blocks.length).forEachOrdered(i -> {
      ...
      blocks[i].setInitPosition(new PVector(
        ((width / 1.9) + gameFrameReference[0].x + blockInitSize.x) + i % BLOCKCOLS * (blockInitSize.x + BLOCKGAP),
        ((height / 4.5) - gameFrameReference[0].y + blockInitSize.y) + i / BLOCKCOLS * (blockInitSize.y + BLOCKGAP)));
    });
  }
    ...
    Arrays.stream(this.blocks).forEach(block -> {
      if (block.alive && block.collider.isCollided(msg.collider)) {
        block.collider.reflectVector(msg.collider).ifPresent(c -> {
          block.toggleAlive();
          senderMap.get(Actor.AC_BALL).send(new CollisionDetectedMessage(Reflecter.RF_BLOCK, c));
        });
      }
    });
  }
  ...
  void sendGameClearMessage() {
    var msg = new GameClearMessage();
    this.senderMap.get(Actor.AC_SYSTEM).send(msg);
  }
}
